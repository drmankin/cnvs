...
create_quiz_desc <- function(n_questions = 7, standard_time_limit = 20, challengr = FALSE){
...
<p>To access the worksheet, you must attend your practical session. In the session, an access code will be announced to unlock the worksheet; you must begin the worksheet within 5 minutes of the access code being released. You will have ", standard_time_limit, " minutes to complete the worksheet, unless you have reasonable adjustments for extra time (", extra_25, " minutes for 25% extra time, and ", extra_50, " minutes for 50% extra time).</p>"
  ...
  return(list(
    ...
    marked = ifelse(challengr == TRUE,
                    ...
                    paste(quiz_desc_marked, quiz_desc_achon)
    )
  ))
}
...
create_quiz_info <- function(module_code, quiz_desc = NULL,
  ...
  # If no description provided, defaults to the convenrhelpr-generated one
  if(is.null(quiz_desc)){
    quiz_desc <- create_quiz_desc()
  }
  ...
  quiz_times <- tibble::tibble(
    ...
    dplyr::mutate(
      dplyr::across(
        dplyr::contains("lock"),
        ~ lubridate::format_ISO8601(.x, usetz = FALSE, precision = "ymdhm"))
    )
...
#'
#' Generate weekly worksheet quizzes for your Canvas site. For making multiple
#' quizzes at once, use in conjunction with [purrr::pmap()] and a tibble of
#' associated quiz info generated by [convenrhelpr::create_quiz_info()].
#'
...
#'
#' @seealso [convenrhelpr::get_module_id()]
#'
...
#'   or "C1234".
#' @param which_week Which weekly quiz do you want to modify?
...
update_quizzes <- function(module_code, which_week,
                           task = c("release_answers", "publish", "change_code", "other"),
                           other_args = list()){
  ...
  quiz_id <- assignments |>
    dplyr::filter(name == paste("Week", which_week, "Worksheet", collapse = " ")) |>
    dplyr::pull(quiz_id)
